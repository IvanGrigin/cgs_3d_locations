...
def main():
    ...
    ap.add_argument("--no-bbox-fallback", action="store_true", default=True, help="Отключить bbox fallback по умолчанию для объектов без mesh")
    ap.add_argument("--bbox-fallback", dest="no_bbox_fallback", action="store_false", help="Включить bbox fallback для debug-рендера")
