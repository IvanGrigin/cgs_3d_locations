...
from src.ChooseObject.wall_material_normalizer import normalize_domlenta_wallpapers_catalog
from src.ChooseObject.wall_material_selector import WallMaterialSelector
from src.pipeline.wall_stage import apply_wall_material_to_scene, load_json, write_json


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Normalize and select wall covering materials.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    normalize = subparsers.add_parser("normalize", help="Normalize Domlenta wallpapers products.csv to JSONL.")
    normalize.add_argument("--products-csv", required=True)
    normalize.add_argument("--out-jsonl", required=True)
    normalize.add_argument("--no-analyze-images", action="store_true")

    select = subparsers.add_parser("select", help="Select one wall material for a prompt/style/room.")
    ...
    apply = subparsers.add_parser("apply-to-scene", help="Apply wall_material.selection.v1.json to scene JSON.")
    apply.add_argument("--scene-json", required=True)
    apply.add_argument("--wall-json", required=True)
...
def main() -> int:
    ...
    if args.command == "normalize":
        materials = normalize_domlenta_wallpapers_catalog(
            Path(args.products_csv),
            Path(args.out_jsonl),
            analyze_images=not bool(args.no_analyze_images),
        )
        print(f"Loaded products: {len(materials)}")
        print(f"Normalized wall materials: {len(materials)}")
        print(f"Saved: {Path(args.out_jsonl)}")
        return 0

    if args.command == "select":
        selector = WallMaterialSelector(Path(args.materials))
        print(f"Loaded wall materials: {len(selector.materials)}")
        ...
        if selected:
            ...
            print(f"average_rgb: {selected.average_rgb} | {selected.average_hex}")
            print(f"dominant_colors_hex: {selected.dominant_colors_hex}")
    ...
    if args.command == "apply-to-scene":
        ...
        wall = load_json(Path(args.wall_json))
        updated = apply_wall_material_to_scene(scene, wall)
