...
# -*- coding: utf-8 -*-
"""
rank_floorplan_candidates_v2.py

Улучшенная версия ранжировщика кандидатов планировок.

Что добавлено по сравнению с v1
===============================
1. Специальный reject для коротких слов / буквенных врезок:
   - примеры типа "НА", "ПУХ";
   - детектируется белая прямоугольная плашка в центре,
     внутри которой несколько крупных тёмных символов.

2. Reject для портретов / лиц:
   - используется OpenCV Haar cascade face detector;
   - для планировок это практически всегда ложная категория,
     поэтому наличие лица сильно штрафуется.

3. Более сильный штраф за "карточки автора проекта":
   - лицо + текстовая колонка + слабая геометрия линий.

4. В CSV/JSONL добавлены новые поля:
   - short_text_box_score
   - face_like_score
   - center_white_box_score
   - center_box_char_count

Пример запуска
==============

python3 src/tools/rank_floorplan_candidates_v2.py \
  --input data/housesru/floorplans_score7_all_pages_parallel/floorplans \
  --out data/housesru/floorplans_ranked_v2 \
  --top-k 800 \
  --mode symlink \
  --preset balanced
...
python3 src/tools/rank_floorplan_candidates_v2.py \
  --input data/housesru/floorplans_score7_all_pages_parallel/floorplans \
  --out data/housesru/floorplans_ranked_v2_recall \
  --top-k 1200 \
  --mode symlink \
  --preset recall
"""
...
class Preset:
    ...
    short_text_hard_reject: float
    face_hard_reject: float
...
def get_preset(name: str) -> Preset:
    if name == "recall":
        return Preset(
            ...
            short_text_hard_reject=0.72,
            face_hard_reject=0.78,
            color_soft_max=0.35,
    ...
    if name == "strict":
        return Preset(
            ...
            short_text_hard_reject=0.52,
            face_hard_reject=0.58,
            color_soft_max=0.14,
    ...
    if name == "balanced":
        return Preset(
            ...
            short_text_hard_reject=0.60,
            face_hard_reject=0.66,
            color_soft_max=0.22,
...
def estimate_face_like(rgb: np.ndarray) -> tuple[float, int, float]:
    ...
    for x, y, fw, fh in faces:
        max_ratio = max(max_ratio, (fw * fh) / area)

    face_count = len(faces)
    count_score = min(1.0, face_count / 2.0)
    size_score = min(1.0, max_ratio / 0.18)

    score = 0.55 * count_score + 0.45 * size_score
    return float(min(1.0, max(0.0, score))), face_count, float(max_ratio)


def detect_center_white_box(rgb: np.ndarray) -> dict[str, Any]:
    gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
    white = (gray >= 228).astype(np.uint8) * 255
    h, w = gray.shape[:2]
    area = max(1, h * w)
    ...
    for i in range(1, num_labels):
        ...
        cx = x + bw / 2.0
        cy = y + bh / 2.0
        dx = abs(cx - center_x) / max(1.0, w / 2.0)
        dy = abs(cy - center_y) / max(1.0, h / 2.0)
        centrality = 1.0 - min(1.0, 0.5 * dx + 0.5 * dy)

        rect_fill = a / max(1, bw * bh)
        score = 0.60 * centrality + 0.40 * rect_fill
...
def estimate_short_text_box(rgb: np.ndarray) -> dict[str, Any]:
    gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
    box = detect_center_white_box(rgb)

    if not box["found"]:
        return {
            "short_text_box_score": 0.0,
            "center_white_box_score": 0.0,
            "center_box_char_count": 0,
            "center_box_dark_ratio": 0.0,
            "center_box_outside_dark_ratio": 0.0,
        }
    ...
    if roi.size == 0:
        return {
            "short_text_box_score": 0.0,
            "center_white_box_score": float(box["score"]),
            "center_box_char_count": 0,
            "center_box_dark_ratio": 0.0,
            "center_box_outside_dark_ratio": 0.0,
        }

    # Тёмные символы на белой плашке
    dark = (roi <= 165).astype(np.uint8) * 255
    num_labels, _labels, stats, _ = cv2.connectedComponentsWithStats(dark, connectivity=8)

    char_count = 0
    total_dark_area = 0
    for i in range(1, num_labels):
        _x, _y, cw, ch, ca = stats[i].tolist()
        if cw < 3 or ch < 8:
            continue
        if ca < 12:
            continue
    ...
    center_dark_ratio = float(total_dark_area / max(1, roi.size))

    outside_mask = np.ones_like(gray, dtype=bool)
    outside_mask[y:y + bh, x:x + bw] = False
    outside_dark_ratio = float(np.count_nonzero(gray[outside_mask] <= 165) / max(1, np.count_nonzero(outside_mask)))

    wide_box = min(1.0, (bw / max(1, bh)) / 4.0)
    char_score = 0.0
    if 1 <= char_count <= 6:
        char_score = 1.0 - abs(char_count - 3) / 5.0
        char_score = max(0.0, char_score)

    dark_inside_score = min(1.0, center_dark_ratio / 0.22)
    dark_outside_score = min(1.0, outside_dark_ratio / 0.30)

    short_text_box_score = (
        0.28 * float(box["score"])
        + 0.20 * wide_box
        + 0.26 * char_score
        + 0.18 * dark_inside_score
        + 0.08 * dark_outside_score
    )

    # Если много H/V линий и высокая grid occupancy, это может быть настоящая планировка.
    return {
        "short_text_box_score": float(min(1.0, max(0.0, short_text_box_score))),
        "center_white_box_score": float(box["score"]),
        "center_box_char_count": int(char_count),
        "center_box_dark_ratio": center_dark_ratio,
        "center_box_outside_dark_ratio": outside_dark_ratio,
    }
...
def analyze(path: Path, max_side: int, preset: Preset) -> dict[str, Any]:
    ...
    face_like_score, face_count, max_face_area_ratio = estimate_face_like(rgb)
    text_box_info = estimate_short_text_box(rgb)
    short_text_box_score = float(text_box_info["short_text_box_score"])
    ...
    author_card_penalty = 0.0
    if face_like_score >= 0.35 and text_like >= 0.35:
        author_card_penalty = 0.65 * min(1.0, 0.55 * face_like_score + 0.45 * text_like)
    ...
    floorplan_score = (
        ...
        - 1.20 * short_text_box_score
        - 1.15 * face_like_score
        - 0.90 * author_card_penalty
    )
    ...
    if short_text_box_score >= preset.short_text_hard_reject:
        hard_reject_reasons.append(f"short_text_box:{short_text_box_score:.4f}>={preset.short_text_hard_reject:.4f}")
    if face_like_score >= preset.face_hard_reject:
        hard_reject_reasons.append(f"face_like:{face_like_score:.4f}>={preset.face_hard_reject:.4f}")
    ...
    return {
        ...
        **{k: (int(v) if isinstance(v, (int, np.integer)) else float(v)) for k, v in lines.items()},
        ...
        "face_like_score": float(face_like_score),
        "face_count": int(face_count),
        "max_face_area_ratio": float(max_face_area_ratio),
        "short_text_box_score": float(short_text_box_score),
        "center_white_box_score": float(text_box_info["center_white_box_score"]),
        "center_box_char_count": int(text_box_info["center_box_char_count"]),
        "center_box_dark_ratio": float(text_box_info["center_box_dark_ratio"]),
        "center_box_outside_dark_ratio": float(text_box_info["center_box_outside_dark_ratio"]),
        "author_card_penalty": float(author_card_penalty),
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Rank floorplan candidates by geometry/topology, including short-text and face rejection.")
...
def main() -> int:
    ...
    manifest = {
        "schema": "floorplan_candidate_ranker/v2",
