...
def normalize_root(raw_url: str) -> Optional[MagazineRoot]:
    ...
    m = NEW_ROOT_RE.search(clean)
    if m:
        year = int(m.group(1))
        slug = m.group(2).strip("/")
        return MagazineRoot(year=year, slug=slug, url=f"https://houses.ru/magazines/{year}/{slug}/", scheme="new_year_slug")

    m = OLD_ROOT_RE.search(clean)
    if m:
        slug = m.group(1).strip("/")
        if not slug or slug.isdigit() or slug == "magazines":
            return None
        year = infer_year_from_slug(slug)
        return MagazineRoot(year=year, slug=slug, url=f"https://houses.ru/magazines/{slug}/", scheme="old_slug")
...
def guess_roots(years: list[int], *, old_100kk: bool, new_100kk: bool, extra: bool) -> list[MagazineRoot]:
    ...
    for url in ["https://houses.ru/magazines/100kk-2022-23/"]:
        root = normalize_root(url)
        if root:
            roots[root.url] = root
